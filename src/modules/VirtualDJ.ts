...
export abstract class VirtualDJ {
  ...
  async listen(interaction: CommandInteraction) {
    ...
    if (!voiceChannelId) {
      await interaction.editReply(":negative_squared_cross_mark: You are not connected to a voice channel.");
      return;
    }
    const guild = await interaction.guild as Guild;
    await interaction.editReply("Attempting to connect to VirtualDJ stream URL...");
    const resource = createAudioResource(`http://${config.virtualdjIP}:8000/listen.mp3`, { inputType: StreamType.Arbitrary });
    ...
    this.player.on('error', error => {
      ...
      interaction.editReply({ content: ":x: An error occurred while playing the VirtualDJ stream", components: [] });
    });
    // Check if audio stream is actually playing.
    try {
      await entersState(this.player, AudioPlayerStatus.Idle, 5_000);
      // The player has entered the idle state within 5 seconds
      console.log("[VIRTUALDJ] Failed to connect");
      const connection = getVoiceConnection(config.guildID);
      if (connection)
        connection.destroy();
      interaction.editReply({ content: ":negative_squared_cross_mark: Failed to connect to the VirtualDJ stream", components: [] });
    } catch (error) {
      // The player has not entered the Idle state and either:
  ...
  stopVirtualDJBtn(interaction: ButtonInteraction) {
    ...
    if (connection)
      connection.destroy();
    interaction.update({ content: ":white_check_mark: Successfully stopped the VirtualDJ stream!", components: [] });
  }
}
